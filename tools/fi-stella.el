...
;;; COPYRIGHT (C) UNIVERSITY OF SOUTHERN CALIFORNIA, 1996-2020
...
;;; IMPORTANT: These extensions are based on fi version 2.0.19 and were
;;; tested up to version 3.23, which means they might not work with
;;; significantly newer versions of the fi Emacs interface.
...
;;; to include the directory this file resides in (for example, let's call it 
;;; `.../stella/tools'), add the following to your .emacs file:
;;;
;;;        (setq load-path (cons ".../stella/tools" load-path))
;;;
...
(defun fi::stella-file-name (lisp-file-name)
  ;; Return the STELLA source file of `lisp-file-name' if it exists.
  (let ((stella-file-name nil))
    (when (and (stringp lisp-file-name)
      ...
      (setq stella-file-name
            (fi:eval-in-lisp "(stella::find-source-from-native-file-name \"%s\")" lisp-file-name))
      (unless (and (stringp stella-file-name)
                   (> (length stella-file-name) 1))
        ;; result with the STELLA NULL string:
...
(defadvice fi::show-found-definition (around stella-frobber act pre com)
  ...
  (let* ((find-file-existing-other-name t)
    ...
    (cond ((and (not (equal filename "TOP-LEVEL"))
                (setq stella-file-name (fi::stella-file-name filename))
