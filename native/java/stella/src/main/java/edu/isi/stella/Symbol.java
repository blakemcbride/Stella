...
public class Symbol extends GeneralizedSymbol {
  ...
  public static boolean cppPossiblyOverloadedFunctionNameP(Symbol functionname) {
    { String fname = functionname.symbolName;
      int dashpos = Native.string_position(fname, '-', 0);
      Stella_Class renamed_Class = null;

      while (dashpos != Stella.NULL_INTEGER) {
        renamed_Class = Stella.string_lookupClass(Native.string_subsequence(fname, 0, dashpos));
        if ((renamed_Class != null) &&
            (!Surrogate.subtypeOfP(renamed_Class.classType, Stella.SGT_STELLA_OBJECT))) {
          return (true);
        }
        dashpos = Native.string_position(fname, '-', dashpos + 1);
      }
      return (false);
    }
  }
  ...
  public Cons cppTranslateAtomicTree() {
    { Symbol tree = this;
      ...
      { GlobalVariable globalvar = tree.softPermanentify().lookupGlobalVariable();
        ...
        if (globalvar != null) {
          return (Cons.list$(Cons.cons(Stella.SYM_STELLA_CPP_IDENT, Cons.cons(GlobalVariable.cppTranslateGlobalVariableName(globalvar), Cons.cons(Stella.NIL, Stella.NIL)))));
        }
        else {
...
;
  ...
  public static Cons cppLookupOperatorTable(Symbol operator) {
    { Cons entry = ((Cons)(Stella.$CPP_OPERATOR_TABLE$.lookup(operator)));
      Cons copiedentry = Stella.NIL;
...
;
  ...
  public static Symbol trueSlotName(Symbol slotname, Surrogate type) {
    return (StorageSlot.canonicalSlotName(((StorageSlot)(Stella_Class.lookupSlot(((Stella_Class)(type.surrogateValue)), slotname)))));
  }
  ...
  public static String yieldStartupFunctionClassname(Symbol functionName) {
    { StringWrapper baseName = StringWrapper.javaTranslateClassNamestring(StringWrapper.wrapString(functionName.symbolName));
      ...
      if (Stella.systemStartupFileP(((String)(Stella.$CURRENTFILENAME$.get())))) {
      ...
      { Keyword key = null;
        ...
        while (iter001.nextP()) {
          ...
          { Keyword testValue000 = ((Keyword)(key));
            ...
            else if (testValue000 == Stella.KWD_C_CALLABLEp) {
              KeyValueList.setDynamicSlotValue(newmethod.dynamicSlots, Stella.SYM_STELLA_METHOD_C_CALLABLEp, (((BooleanWrapper)(value)).wrapperValue ? Stella.TRUE_WRAPPER : Stella.FALSE_WRAPPER), Stella.FALSE_WRAPPER);
            }
            else {
