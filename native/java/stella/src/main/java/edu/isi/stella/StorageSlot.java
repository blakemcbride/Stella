...
;
  ...
  public static Cons yieldStorageSlotCSignature(StorageSlot slot, StandardObject firstargtype) {
    { Cons sig = StorageSlot.yieldStorageSlotCppSignature(slot, firstargtype, true);
  ...
  public static Cons yieldStorageSlotCppSignature(StorageSlot slot, StandardObject firstargtype, boolean qualifyP) {
    { Object old$Module$000 = Stella.$MODULE$.get();
      Object old$Context$000 = Stella.$CONTEXT$.get();

      try {
        Native.setSpecial(Stella.$MODULE$, (qualifyP ? Stella.$ROOT_MODULE$ : slot.homeModule()));
        ...
        { Symbol slotname = Symbol.trueSlotName(slot.slotName, slot.slotOwner);
          ...
          Cons cppsig = Cons.cons(Cons.cons(Symbol.cppTranslateName(slotname), Cons.cons(StandardObject.cppTranslateAndPointerizeTypeSpec(slottype), Cons.cons(StringWrapper.wrapString(Native.stringify(StandardObject.typeSpecToBaseType(slottype))), Stella.NIL))), Stella.NIL);
...
;
  ...
  /** Return the accessor code to read <code>slot</code> on an object of type <code>renamed_Class</code>,
   * or NULL if no such accessor exists.  In that case, print a warning if <code>warnP</code> is TRUE.
   * NOTE: the accessor expects <code>canonicalSlotName</code>s (which see).
   * @param slot
   * @param renamed_Class
   * @param warnP
   * @return java.lang.reflect.Method
   */
  public static java.lang.reflect.Method getGenericSlotAccessor(StorageSlot slot, Stella_Class renamed_Class, boolean warnP) {
    slot = StorageSlot.canonicalSlot(slot);
    { boolean slotvisibleP = Stella_Class.subclassOfP(renamed_Class, ((Stella_Class)(slot.slotOwner.surrogateValue)));
      Stella_Class nativehome = Slot.nativeSlotHome(slot, renamed_Class);
      boolean slotreadableP = (nativehome != null) &&
          StorageSlot.genericSlotReaderP(slot, nativehome);
      java.lang.reflect.Method accessorcode = ((slotvisibleP &&
          slotreadableP) ? Slot.nativeSlotHome(slot, renamed_Class).classSlotAccessorCode : ((java.lang.reflect.Method)(null)));

      if ((accessorcode == null) &&
          warnP) {
        if (slotvisibleP) {
          Stella.STANDARD_WARNING.nativeStream.println("Warning: No generic slot accessor available for `" + slot + "'");
        }
        else {
          Stella.STANDARD_WARNING.nativeStream.println("Warning: Slot `" + slot + "' does not exist on objects of type `" + renamed_Class.classType + "'");
        }
      }
      return (accessorcode);
    }
  ...
  public static boolean genericSlotWriterP(StorageSlot slot, Stella_Class renamed_Class) {
    return ((!slot.slotHardwiredP) &&
        StorageSlot.genericSlotReaderP(slot, renamed_Class));
  }

  public static boolean genericSlotReaderP(StorageSlot slot, Stella_Class renamed_Class) {
    if ((!slot.abstractP) &&
        (Surrogate.subtypeOfP(slot.slotOwner, Stella.SGT_STELLA_OBJECT) &&
         (Slot.nativeSlotHome(slot, renamed_Class) == renamed_Class))) {
      { Surrogate type = Surrogate.canonicalType(StandardObject.typeSpecToBaseType(StorageSlot.computeRealSlotType(slot, renamed_Class.classType, Surrogate.canonicalType(slot.type()))));
...
;
   ...
   * @return Symbol
   */
  public static Symbol canonicalSlotName(StorageSlot slot) {
    return (StorageSlot.canonicalSlot(slot).slotName);
  }
