...
public class SystemDefinition extends StandardObject {
  ...
  public static String javaYieldSystemParentName(SystemDefinition system) {
    system = system;
    { String parentname = Stella.fileNameWithoutDirectory(Stella.directoryFileName(Stella.directoryParentDirectory(Stella.rootNativeDirectory(), 1)));

      if (Stella.blankStringP(parentname)) {
        parentname = "stella";
      }
      return (parentname + "-parent-system");
    }
  }

  public static String javaYieldSystemVersion(SystemDefinition system) {
    return (SystemDefinition.getSystemVersionString(system));
  }

  public static String javaYieldSystemGroupName(SystemDefinition system) {
    if (system.systemGroupName() != null) {
      return (system.systemGroupName());
    }
    else {
      { String defaultgroupname = Module.javaPackagePrefix(Stella.getStellaModule(system.cardinalModule, true), ".");
        int pos = 0;

        defaultgroupname = Native.string_subsequence(defaultgroupname, 0, defaultgroupname.length() - 1);
        pos = Native.string_lastPosition(defaultgroupname, '.', Stella.NULL_INTEGER);
        if (pos != Stella.NULL_INTEGER) {
          defaultgroupname = Native.string_subsequence(defaultgroupname, 0, pos);
        }
  ...
  public String systemGroupName() {
    { SystemDefinition self = this;
      ...
      { String answer = ((StringWrapper)(KeyValueList.dynamicSlotValue(self.dynamicSlots, Stella.SYM_STELLA_SYSTEM_GROUP_NAME, Stella.NULL_STRING_WRAPPER))).wrapperValue;
  ...
  /** Compute a version string for <code>system</code> which is either based on an explicit :version
   * annotation on <code>system</code>s definition, the readable value of an existing <code>$LSYSTEMG_VERSIONJ_STRINGK$</code>,
   * variable or on its most recent modification date of any of its source files.
   * @param system
   * @return String
   */
  public static String getSystemVersionString(SystemDefinition system) {
    { String name = system.name;
      Module module = Stella.getStellaModule(system.cardinalModule, false);
      String versionsymbolname = null;
      Symbol versionsymbol = null;
      GlobalVariable versionvariable = null;
      String versionstring = ((StringWrapper)(KeyValueList.dynamicSlotValue(system.dynamicSlots, Stella.SYM_STELLA_SYSTEM_VERSION, Stella.NULL_STRING_WRAPPER))).wrapperValue;

      if ((versionstring == null) &&
          (module != null)) {
        { Stella_Object pattern = null;
          Cons iter000 = Cons.consList(Cons.cons(StringWrapper.wrapString("*<name>-version*"), Cons.cons(StringWrapper.wrapString("*<name>-version-string*"), Stella.NIL)));

          loop000 : for (;!(iter000 == Stella.NIL); iter000 = iter000.rest) {
            pattern = iter000.value;
            versionsymbolname = Stella.replaceSubstrings(((StringWrapper)(pattern)).wrapperValue, name, "<name>");
            if (!(module.caseSensitiveP)) {
              versionsymbolname = Native.stringUpcase(versionsymbolname);
            }
            versionsymbol = Symbol.lookupSymbolInModule(versionsymbolname, module, false);
            if (versionsymbol != null) {
              versionvariable = versionsymbol.lookupGlobalVariable();
              if ((versionvariable != null) &&
                  ((versionvariable.type() == Stella.SGT_STELLA_STRING) &&
                   (GlobalVariable.readGlobalVariableValue(versionvariable) != null))) {
                versionstring = ((StringWrapper)(GlobalVariable.readGlobalVariableValue(versionvariable))).wrapperValue;
                if (versionstring != null) {
                  break loop000;
                }
              }
      ...
      if (versionstring == null) {
        versionstring = CalendarDate.dateToVersionString(SystemDefinition.getSystemLastModifiedDate(system), Stella.KWD_SECONDS);
      }
      return (((StringWrapper)(Stella.splitString(versionstring, ' ').last())).wrapperValue);
    }
  }

  /** Compute a last-modified date for <code>system</code> based on its most recently modified source file.
   * @param system
   * @return CalendarDate
   */
  public static CalendarDate getSystemLastModifiedDate(SystemDefinition system) {
    { CalendarDate date = null;
      ...
      { Object old$Currentsystemdefinition$000 = Stella.$CURRENTSYSTEMDEFINITION$.get();
        ...
        try {
          Native.setSpecial(Stella.$CURRENTSYSTEMDEFINITION$, Stella.getSystemDefinition(system.name, true));
          Native.setSpecial(Stella.$CURRENTSYSTEMDEFINITIONSUBDIRECTORY$, ((SystemDefinition)(Stella.$CURRENTSYSTEMDEFINITION$.get())).directory);
          {
            { StringWrapper file = null;
              ...
              for (;!(iter000 == Stella.NIL); iter000 = iter000.rest) {
                ...
                file = StringWrapper.wrapString(Stella.makeFileName(file.wrapperValue, Stella.KWD_STELLA, true));
                if (Stella.probeFileP(file.wrapperValue)) {
                  if ((date == null) ||
                      Stella.fileWriteDate(file.wrapperValue).greaterP(date)) {
                    date = Stella.fileWriteDate(file.wrapperValue);
                  }
                }
              }
            }
...
;
  ...
  public static Cons collectStartupFormsFromSystemFile(SystemDefinition system) {
    { String systemfilename = system.definitionFile;
      Module module = null;
...
;
   ...
   * in case they were not explicitly specified in <code>system</code>s definition.  Raises an error in case
   * any of those root directories is missing and can't be computed.
   * @param system
   */
  public static void computeSystemRootDirectories(SystemDefinition system) {
    { String systemrootdir = SystemDefinition.findSystemRootDirectory(system);
        ...
        { SystemDefinition value000 = null;
          ...
          { SystemDefinition sys = null;
            ...
            loop000 : for (;!(iter000 == Stella.NIL); iter000 = iter000.rest) {
              ...
              if (Stella.stringEqlP(SystemDefinition.findSystemRootDirectory(sys), systemrootdir)) {
                value000 = sys;
                break loop000;
              }
            }
          ...
          { SystemDefinition temp000 = value000;
            ...
            { SystemDefinition referencesystem = ((temp000 != null) ? temp000 : system);

              { String temp001 = referencesystem.sourceRootDirectory;

                { String rootsourcedefault = ((temp001 != null) ? temp001 : Stella.concatenateFileNames(systemrootdir, Cons.cons(StringWrapper.wrapString(Stella.relativeUnlogicalizePathname(((String)(Stella.$DEFAULTROOTSOURCEDIRECTORY$.get())))), Stella.NIL)));

                  { String temp002 = referencesystem.nativeRootDirectory;

                    { String rootnativedefault = Stella.translateLogicalPathname(((temp002 != null) ? temp002 : Stella.concatenateFileNames(systemrootdir, Cons.cons(StringWrapper.wrapString(Stella.relativeUnlogicalizePathname(((String)(Stella.$DEFAULTROOTNATIVEDIRECTORY$.get())))), Stella.NIL))));

                      { String temp003 = referencesystem.binaryRootDirectory;

                        { String rootbinarydefault = Stella.translateLogicalPathname(((temp003 != null) ? temp003 : Stella.concatenateFileNames(systemrootdir, Cons.cons(StringWrapper.wrapString(Stella.relativeUnlogicalizePathname(((String)(Stella.$DEFAULTROOTBINARYDIRECTORY$.get())))), Stella.NIL))));

                          { Object old$Defaultrootsourcedirectory$000 = Stella.$DEFAULTROOTSOURCEDIRECTORY$.get();
                            Object old$Defaultrootnativedirectory$000 = Stella.$DEFAULTROOTNATIVEDIRECTORY$.get();
                            Object old$Defaultrootbinarydirectory$000 = Stella.$DEFAULTROOTBINARYDIRECTORY$.get();

                            try {
                              Native.setSpecial(Stella.$DEFAULTROOTSOURCEDIRECTORY$, rootsourcedefault);
                              Native.setSpecial(Stella.$DEFAULTROOTNATIVEDIRECTORY$, rootnativedefault);
                              Native.setSpecial(Stella.$DEFAULTROOTBINARYDIRECTORY$, rootbinarydefault);
                              if (system.sourceRootDirectory == null) {
                                system.sourceRootDirectory = Stella.rootSourceDirectory();
                              }
                              if (system.nativeRootDirectory == null) {
                                system.nativeRootDirectory = Stella.rootNativeDirectory();
                              }
                              if (system.binaryRootDirectory == null) {
                                system.binaryRootDirectory = Stella.rootBinaryDirectory();
                              }

                            } finally {
                              Stella.$DEFAULTROOTBINARYDIRECTORY$.set(old$Defaultrootbinarydirectory$000);
                              Stella.$DEFAULTROOTNATIVEDIRECTORY$.set(old$Defaultrootnativedirectory$000);
                              Stella.$DEFAULTROOTSOURCEDIRECTORY$.set(old$Defaultrootsourcedirectory$000);
                            }
                          }
                        }
                      }
      ...
      if ((system.sourceRootDirectory == null) ||
        ...
        { OutputStringStream stream000 = OutputStringStream.newOutputStringStream();
          ...
          stream000.nativeStream.print("Failed to define or compute one or more of `" + system.name + "''s root directories");
          throw ((StellaException)(StellaException.newStellaException(stream000.theStringReader()).fillInStackTrace()));
        }
      }
    }
  ...
  /** Given a system definition <code>system</code> try to determine the root directory
   * of the STELLA installation containing this system based on its definition file.
   * @param system
   * @return String
   */
  public static String findSystemRootDirectory(SystemDefinition system) {
    { String sysdeffile = system.definitionFile;

      if (sysdeffile != null) {
        return (Stella.findSystemRootDirectoryFromFile(sysdeffile));
      }
      else {
        return (null);
      }
  ...
  public static Stella_Object accessSystemDefinitionSlotValue(SystemDefinition self, Symbol slotname, Stella_Object value, boolean setvalueP) {
    ...
    else if (slotname == Stella.SYM_STELLA_DEFINITION_FILE) {
      if (setvalueP) {
        self.definitionFile = ((StringWrapper)(value)).wrapperValue;
      }
      else {
        value = StringWrapper.wrapString(self.definitionFile);
      }
    }
