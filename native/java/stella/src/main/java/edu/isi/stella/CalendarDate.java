...
;
  ...
  /** Transform <code>date</code> into a version string according to <code>granularity</code>
   * which is at least :days and defaults to :seconds.
   * @param date
   * @param granularity
   * @return String
   */
  public static String dateToVersionString(CalendarDate date, Keyword granularity) {
    { String formatstring = "%Y.%m.%d.%H%M%S";

      if (granularity == Stella.KWD_DAYS) {
        formatstring = "%Y.%m.%d";
      }
      else if (granularity == Stella.KWD_HOURS) {
        formatstring = "%Y.%m.%d.%H";
      }
      else if (granularity == Stella.KWD_MINUTES) {
        formatstring = "%Y.%m.%d.%H%M";
      }
      else if (granularity == Stella.KWD_SECONDS) {
        formatstring = "%Y.%m.%d.%H%M%S";
      }
      else {
      }
      return (Stella_Object.formatDate(date, null, formatstring));
    }
  }
