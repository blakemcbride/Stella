    ...
    { Object old$Module$000 = Stella.$MODULE$.get();
      ...
      try {
        ...
        if (Stella.currentStartupTimePhaseP(4)) {
          Stella.$STELLA_MAJOR_VERSION_NUMBER$ = ((int)(Native.stringToInteger(((StringWrapper)(Stella.splitString(Stella.$STELLA_VERSION$, '.').nth(0))).wrapperValue)));
          Stella.$STELLA_MINOR_VERSION_NUMBER$ = ((int)(Native.stringToInteger(((StringWrapper)(Stella.splitString(Stella.$STELLA_VERSION$, '.').nth(1))).wrapperValue)));
          Stella.$STELLA_PATCH_LEVEL$ = ((int)(Native.stringToInteger(((StringWrapper)(Stella.splitString(Stella.$STELLA_VERSION$, '.').nth(2))).wrapperValue)));
          { StringWrapper temp089 = ((StringWrapper)(Stella.splitString(Stella.$STELLA_VERSION$, '.').nth(3)));

            Stella.$STELLA_RELEASE_STATE$ = ((temp089 != null) ? temp089.wrapperValue : "");
          }
        ...
        if (Stella.currentStartupTimePhaseP(7)) {
          ...
          Stella.defineFunctionObject("STARTUP", "(DEFUN STARTUP ((VERBOSE? BOOLEAN)) :PUBLIC? TRUE :C-CALLABLE? TRUE)", Native.find_java_method("edu.isi.stella.Stella", "startup", new java.lang.Class [] {java.lang.Boolean.TYPE}), null);
        ...
        if (Stella.currentStartupTimePhaseP(9)) {
          ...
          Stella.defineGlobalVariableObject("(DEFGLOBAL *STELLA-VERSION* STRING \"3.7.0\" :PUBLIC? TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *STELLA-MAJOR-VERSION-NUMBER* INTEGER (STRING-TO-INTEGER (NTH (SPLIT-STRING *STELLA-VERSION* #\\.) 0)) :PUBLIC? TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *STELLA-MINOR-VERSION-NUMBER* INTEGER (STRING-TO-INTEGER (NTH (SPLIT-STRING *STELLA-VERSION* #\\.) 1)) :PUBLIC? TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *STELLA-PATCH-LEVEL* INTEGER (STRING-TO-INTEGER (NTH (SPLIT-STRING *STELLA-VERSION* #\\.) 2)) :PUBLIC? TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *STELLA-RELEASE-STATE* STRING (FIRST-DEFINED (NTH (SPLIT-STRING *STELLA-VERSION* #\\.) 3) \"\") :PUBLIC? TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *STELLA-VERSION-STRING* STRING (STELLA-VERSION-STRING) :PUBLIC? TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *STELLA-GC-VERSION* STRING \"not-installed\" :DOCUMENTATION \"Run-time version of the GC library used by STELLA.\" :PUBLIC? TRUE)", null);
          { int version = 0;

            Stella.$STELLA_GC_VERSION$ = Native.integerToString(((long)((((version >> 16)) & 255)))) + "." + Native.integerToString(((long)((((version >> 8)) & 255)))) + "." + Native.integerToString(((long)((version & 255))));
          }
          Stella.defineGlobalVariableObject("(DEFGLOBAL *STARTUP-TIME-PHASES* (LIST OF KEYWORD) (CAST (LIST :EARLY-INITS :MODULES :SYMBOLS :QUOTED-CONSTANTS :GLOBALS :CLASSES :FINALIZE-CLASSES :METHODS :FINALIZE-METHODS :FINAL) (LIST OF KEYWORD)) :DOCUMENTATION \"List of phases that can be legally used as an optional\nphase argument to a `startup-time-progn' form.  The corresponding code\nwill be executed during the execution of a startup-time-code function only\nif the position of the keyword in the list corresponds to the current value of\n`*STARTUP-TIME-PHASE*', or if phasing of startup-time code is disabled.\")", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *STARTUP-TIME-PHASE* INTEGER 999 :PUBLIC? TRUE :DOCUMENTATION \"The current phase during 'phased startup'.\nThe value has to correspond to the position of one of the keywords\nin `*STARTUP-TIME-PHASES*'.  999 means no phasing at all.\")", null);
        }
