...
public class Keyword extends GeneralizedSymbol {
          ...
          { GlobalVariable demon000 = ((GlobalVariable)(demon));
            ...
            { MethodSlot setter = GlobalVariable.lookupVariableDemonSetter(demon000);
              ...
              if (setter != null) {
                ...
                GlobalVariable.writeGlobalVariableValue(demon000, coercedvalue);
              }
            }
          }
        }
   ...
   * for files of <code>type</code>.  If <code>type</code> does not naturally suggest which native language
   * it is associated with, key in on the current translation output language.
   * Returns the empty string for unhandled types.
   * @param type
   * @return String
   */
  public static String getNativeSourceSubdirectoryFromFileType(Keyword type) {
    { String directory = "";
      boolean javaP = (type == Stella.KWD_JAVA) ||
      ...
      if (javaP) {
        if (Stella.$JAVA_BUILD_METHOD$ == Stella.KWD_MAVEN) {
          directory = Stella.concatenateFileNames("src", Cons.cons(StringWrapper.wrapString("main"), Cons.cons(StringWrapper.wrapString("java"), Stella.NIL)));
        }
        else {
        }
        directory = Stella.concatenateFileNames(directory, Cons.cons(StringWrapper.wrapString(Module.javaPackagePrefix(((Module)(Stella.$MODULE$.get())), Stella.directorySeparatorString())), Stella.NIL));
      }
      return (directory);
    }
  }
   ...
   * @return String
   */
  public static String getSystemSubdirectoryFromFileType(Keyword type) {
    { String systemdirectory = "";
      ...
      if (javaP) {
        if (Stella.$JAVA_BUILD_METHOD$ == Stella.KWD_JAVAC) {
          return ("");
        }
        else if (Stella.$JAVA_BUILD_METHOD$ == Stella.KWD_MAVEN) {
          if (((SystemDefinition)(Stella.$CURRENTSYSTEMDEFINITION$.get())) != null) {
            return (((SystemDefinition)(Stella.$CURRENTSYSTEMDEFINITION$.get())).name);
          }
        }
        else {
        }
      }
      if (!(Stella.blankStringP(((String)(Stella.$CURRENTSYSTEMDEFINITIONSUBDIRECTORY$.get()))))) {
        systemdirectory = ((String)(Stella.$CURRENTSYSTEMDEFINITIONSUBDIRECTORY$.get()));
      }
      if (Stella.blankStringP(systemdirectory) &&
          (((SystemDefinition)(Stella.$CURRENTSYSTEMDEFINITION$.get())) != null)) {
        systemdirectory = ((SystemDefinition)(Stella.$CURRENTSYSTEMDEFINITION$.get())).name;
      }
   ...
   * be used for files of <code>type</code>.  If <code>type</code> does not naturally suggest which native
   * language it is associated with, key in on the current translation output language.
   * Returns the empty string for unhandled types.
   * @param type
   * @return String
   */
  public static String getLanguageSubdirectoryFromFileType(Keyword type) {
    { String splitterpath = "";
      ...
      if ((type == Stella.KWD_LISP) ||
        ...
        splitterpath = Stella.$CPP_SPLITTER_PATH$;
          ...
          (type == Stella.KWD_OTHER)) {
        return (Keyword.getLanguageSubdirectoryFromFileType(((Keyword)(Stella.$TRANSLATOROUTPUTLANGUAGE$.get()))));
      }
      else {
      }
      return (splitterpath);
  ...
  /** Given a normalized file <code>type</code>, determine which STELLA installation
   * root directory tree it naturally belongs to (i.e., source, native or binary).
   * @param type
   * @return String
   */
  public static String getRootDirectoryFromFileType(Keyword type) {
    { String root = null;
