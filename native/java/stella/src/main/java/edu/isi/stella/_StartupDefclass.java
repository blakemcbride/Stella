  ...
  static void helpStartupDefclass2() {
    {
      ...
      Stella.defineFunctionObject("CANONICAL-SLOT", "(DEFUN (CANONICAL-SLOT STORAGE-SLOT) ((SLOT STORAGE-SLOT)) :DOCUMENTATION \"Computes the slot at the base of a renaming chain for 'slot'.\" :PUBLIC? TRUE)", Native.find_java_method("edu.isi.stella.StorageSlot", "canonicalSlot", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.StorageSlot")}), null);
      Stella.defineFunctionObject("CANONICAL-SLOT-NAME", "(DEFUN (CANONICAL-SLOT-NAME SYMBOL) ((SLOT STORAGE-SLOT)) :DOCUMENTATION \"Name of the slot at the base of a renaming chain for 'slot'.\" :PUBLIC? TRUE)", Native.find_java_method("edu.isi.stella.StorageSlot", "canonicalSlotName", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.StorageSlot")}), null);
  ...
  static void helpStartupDefclass3() {
    {
      ...
      Stella.defineMethodObject("(DEFMETHOD (DELETED? BOOLEAN) ((SELF RECYCLE-LIST)))", MethodCodeWrapper.wrapMethodCode(Native.find_java_method("edu.isi.stella.RecycleList", "deletedP", new java.lang.Class [] {})), null);
      Stella.defineMethodObject("(DEFMETHOD (DELETED?-SETTER BOOLEAN) ((SELF RECYCLE-LIST) (VALUE BOOLEAN)))", MethodCodeWrapper.wrapMethodCode(Native.find_java_method("edu.isi.stella.RecycleList", "deletedPSetter", new java.lang.Class [] {java.lang.Boolean.TYPE})), null);
      ...
      Stella.defineMethodObject("(DEFMETHOD SWEEP ((SELF OBJECT)) :DOCUMENTATION \"Default method.  Sweep up all `self'-type objects.\")", MethodCodeWrapper.wrapMethodCode(Native.find_java_method("edu.isi.stella.Stella_Object", "sweep", new java.lang.Class [] {})), null);
      ...
      Stella.defineFunctionObject("GENERIC-SLOT-READER?", "(DEFUN (GENERIC-SLOT-READER? BOOLEAN) ((SLOT STORAGE-SLOT) (CLASS CLASS)))", Native.find_java_method("edu.isi.stella.StorageSlot", "genericSlotReaderP", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.StorageSlot"), Native.find_java_class("edu.isi.stella.Stella_Class")}), null);
      Stella.defineFunctionObject("GENERIC-SLOT-WRITER?", "(DEFUN (GENERIC-SLOT-WRITER? BOOLEAN) ((SLOT STORAGE-SLOT) (CLASS CLASS)))", Native.find_java_method("edu.isi.stella.StorageSlot", "genericSlotWriterP", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.StorageSlot"), Native.find_java_class("edu.isi.stella.Stella_Class")}), null);
      ...
      Stella.defineFunctionObject("GET-GENERIC-SLOT-ACCESSOR", "(DEFUN (GET-GENERIC-SLOT-ACCESSOR FUNCTION-CODE) ((SLOT STORAGE-SLOT) (CLASS CLASS) (WARN? BOOLEAN)) :DOCUMENTATION \"Return the accessor code to read `slot' on an object of type `class',\nor NULL if no such accessor exists.  In that case, print a warning if `warn?' is TRUE.\nNOTE: the accessor expects `canonical-slot-name's (which see).\" :PUBLIC? TRUE)", Native.find_java_method("edu.isi.stella.StorageSlot", "getGenericSlotAccessor", new java.lang.Class [] {Native.find_java_class("edu.isi.stella.StorageSlot"), Native.find_java_class("edu.isi.stella.Stella_Class"), java.lang.Boolean.TYPE}), null);
    ...
    { Object old$Module$000 = Stella.$MODULE$.get();
      ...
      try {
        ...
        if (Stella.currentStartupTimePhaseP(9)) {
          ...
          Stella.defineGlobalVariableObject("(DEFSPECIAL *RECYCLING-ENABLED?* BOOLEAN TRUE :DOCUMENTATION \"If `TRUE' calls to `new' or `free' on classes with :recycle-method;\nspecifications will actually operate on recycle lists.  Otherwise, all\nrecycle list operations will be no-ops.\" :PUBLIC? TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *ALL-RECYCLE-LISTS* (LIST OF RECYCLE-LIST) NULL)", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *MAX-RECYCLE-LIST-BYTES* INTEGER 3000000 :DOCUMENTATION \"Maximum number of bytes to be occupied by recycle lists.\" :PUBLIC? TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *RECYCLE-LISTS-MAINTENANCE-INTERVAL* INTEGER 5000 :DOCUMENTATION \"Total number of recycle list extensions after which a\nsize maintenance operation is performed to ensure that\nthe `*MAX-RECYCLE-LIST-BYTES*' limit is still met.\" :PUBLIC? TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *RECYCLE-LISTS-MAINTENANCE-TIMER* INTEGER *RECYCLE-LISTS-MAINTENANCE-INTERVAL* :PUBLIC? TRUE)", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *CURRENTLY-RECYCLED-ITEMS* (HASH-TABLE OF OBJECT OBJECT) NULL :DOCUMENTATION \"During recycle list debugging holds all items that live in\nsome free list.  Depending on program behavior, this table can get quite large!\")", null);
          Stella.defineGlobalVariableObject("(DEFGLOBAL *TRANSLATE-WITH-RECYCLE-LIST-DEBUGGING?* BOOLEAN FALSE)", null);
        }
