...
(CL:DEFMETHOD THE-ARRAY-READER ((SELF 1D-ARRAY))
  (CL:RETURN-FROM THE-ARRAY-READER (%THE-ARRAY SELF)))
...
;;; (DEFCLASS BUFFERED-STRING ...)

(CL:DEFCLASS BUFFERED-STRING (STANDARD-OBJECT)
  ((BUFFER :TYPE CL:SIMPLE-STRING :INITFORM STELLA::NULL-STRING
    :ALLOCATION :INSTANCE :ACCESSOR %BUFFER)
...
(CL:DEFUN NEW-BUFFERED-STRING ()
  (CL:LET* ((SELF NULL))
   (CL:SETQ SELF (CL:MAKE-INSTANCE (CL:QUOTE BUFFERED-STRING)))
   (CL:SETF (%FILL-POINTER SELF) 0) (CL:SETF (%BUFFER-SIZE SELF) 16)
   (CL:SETF (%BUFFER SELF) (MAKE-RAW-MUTABLE-STRING 16))
...
(CL:DEFUN ACCESS-BUFFERED-STRING-SLOT-VALUE (SELF SLOTNAME VALUE SETVALUE?)
  (CL:COND
   ((CL:EQ SLOTNAME SYM-COLLECTIONS-STELLA-BUFFER)
    (CL:IF SETVALUE? (CL:SETF (%BUFFER SELF) (%WRAPPER-VALUE VALUE))
     (CL:SETQ VALUE (WRAP-MUTABLE-STRING (%BUFFER SELF)))))
...
than the current size of `self', it's buffer will be truncated."
  ...
  (CL:CHECK-TYPE SIZE CL:FIXNUM)
  (CL:LET*
   ((OLDSIZE (%BUFFER-SIZE SELF)) (OLDBUFFER (%BUFFER SELF))
    (NEWBUFFER OLDBUFFER))
   ...
   (CL:COND
    ((CL:= OLDSIZE 0)
     ...
     (CL:SETF (%BUFFER SELF) NEWBUFFER)
     (CL:SETF (%BUFFER-SIZE SELF) SIZE))
    ((CL:= SIZE OLDSIZE))
    (CL:T
     (CL:LET* ((L (%FILL-POINTER SELF)))
      (CL:DECLARE (CL:TYPE CL:FIXNUM L))
      (CL:SETQ NEWBUFFER (MAKE-RAW-MUTABLE-STRING SIZE))
      (CL:SETF (%BUFFER SELF) NEWBUFFER)
      (CL:SETF (%BUFFER-SIZE SELF) SIZE)
      (CL:WHEN (CL:< SIZE L) (CL:SETQ L SIZE)
       (CL:SETF (%FILL-POINTER SELF) L))
...
(CL:DEFMETHOD ENSURE-FREE-SPACE ((SELF BUFFERED-STRING) SIZE)
  ...
  (CL:CHECK-TYPE SIZE CL:FIXNUM)
  (CL:COND
   ((CL:> (CL:+ (%FILL-POINTER SELF) SIZE) (%BUFFER-SIZE SELF))
    (CL:RETURN-FROM ENSURE-FREE-SPACE
     (RESIZE-BUFFER SELF
      (MAX (CL:* (%BUFFER-SIZE SELF) 2)
       (CL:+ (%FILL-POINTER SELF) SIZE)))))
   (CL:T (CL:RETURN-FROM ENSURE-FREE-SPACE (%BUFFER SELF)))))
...
(CL:DEFMETHOD FREE-SPACE ((SELF BUFFERED-STRING))
  ...
  (CL:RETURN-FROM FREE-SPACE
   (CL:- (%BUFFER-SIZE SELF) (%FILL-POINTER SELF))))
   ...
   (CL:COND
    ((CL:< NEWLENGTH 0)
     (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
      ...
      (CL:ERROR
       (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000)))))
    ((CL:< NEWLENGTH FP) (CL:SETF (%FILL-POINTER SELF) NEWLENGTH))
    ((CL:> NEWLENGTH FP) (ENSURE-FREE-SPACE SELF (CL:- NEWLENGTH FP))
     (CL:LET* ((BUF (%BUFFER SELF)))
      ...
      (CL:LOOP WHILE (CL:< FP NEWLENGTH) DO
       (CL:LET ((SELF BUF) (CH NULL-CHARACTER) (POSITION FP))
        ...
        (SETF
         ...
         (CL:THE CL:CHARACTER CH)))
       (CL:SETQ FP (CL:1+ FP)))
      (CL:SETF (%FILL-POINTER SELF) NEWLENGTH))))
...
(CL:DEFMETHOD NON-EMPTY? ((SELF BUFFERED-STRING))
  "Return `true' if `self' has length > 0."
  (CL:RETURN-FROM NON-EMPTY? (> (%FILL-POINTER SELF) 0)))
...
(CL:DEFMETHOD NTH ((SELF BUFFERED-STRING) POSITION)
  ...
  (CL:CHECK-TYPE POSITION CL:FIXNUM)
  (CL:RETURN-FROM NTH
   (CL:LET ((SELF (%BUFFER SELF)) (POSITION POSITION))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SELF)
     (CL:TYPE CL:FIXNUM POSITION))
    (CL:SCHAR (CL:THE CL:SIMPLE-STRING SELF)
     (CL:THE CL:FIXNUM POSITION)))))
...
(CL:DEFMETHOD NTH-SETTER ((SELF BUFFERED-STRING) CH POSITION)
  ...
  (CL:CHECK-TYPE POSITION CL:FIXNUM)
  (CL:RETURN-FROM NTH-SETTER
   (CL:LET ((SELF (%BUFFER SELF)) (CH CH) (POSITION POSITION))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SELF)
     (CL:TYPE CL:FIXNUM POSITION))
    (SETF
     (CL:SCHAR (CL:THE CL:SIMPLE-STRING SELF)
...
(CL:DEFMETHOD LAST ((SELF BUFFERED-STRING))
  ...
  (CL:RETURN-FROM LAST
   (CL:LET
    ((SELF (%BUFFER SELF)) (POSITION (CL:1- (%FILL-POINTER SELF))))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SELF)
     (CL:TYPE CL:FIXNUM POSITION))
    (CL:SCHAR (CL:THE CL:SIMPLE-STRING SELF)
     (CL:THE CL:FIXNUM POSITION)))))
...
(CL:DEFMETHOD LAST-SETTER ((SELF BUFFERED-STRING) VALUE)
  (CL:RETURN-FROM LAST-SETTER
   (CL:LET
    ((SELF (%BUFFER SELF)) (CH VALUE)
     (POSITION (CL:1- (%FILL-POINTER SELF))))
    (CL:DECLARE (CL:TYPE CL:SIMPLE-STRING SELF)
     (CL:TYPE CL:FIXNUM POSITION))
    (SETF
     (CL:SCHAR (CL:THE CL:SIMPLE-STRING SELF)
...
Resize the buffer if necessary."
  ...
  (CL:LET*
   ((VALLENGTH (CL:THE CL:FIXNUM (CL:LENGTH VALUE))) (VALCURSOR 0)
    (BUFSIZE (%BUFFER-SIZE SELF)) (BUFCURSOR (%FILL-POINTER SELF))
    (BUFFER (%BUFFER SELF)))
   (CL:DECLARE
   ...
   (CL:SETF (%FILL-POINTER SELF) BUFCURSOR))
  :VOID)
...
(CL:DEFMETHOD COPY ((SELF BUFFERED-STRING))
  (CL:LET* ((SELF-000 (NEW-BUFFERED-STRING)))
   (CL:SETF (%BUFFER-SIZE SELF-000) (%BUFFER-SIZE SELF))
   (CL:SETF (%FILL-POINTER SELF-000) (%FILL-POINTER SELF))
   (CL:LET* ((COPY SELF-000))
    (CL:SETF (%BUFFER COPY)
     (CL:THE CL:SIMPLE-STRING
      (CL:COPY-SEQ
       (CL:THE CL:SIMPLE-STRING
        (SUBSEQUENCE (%BUFFER SELF) 0 NULL-INTEGER)))))
    (CL:RETURN-FROM COPY COPY))))
...
`end' value of NULL stands for the rest of the string."
  (CL:RETURN-FROM THE-STRING
   (SUBSEQUENCE (%BUFFER SELF) 0 (%FILL-POINTER SELF))))
...
`end' value of NULL stands for the rest of the string."
  ...
  #+MCL
  (CL:CHECK-TYPE END CL:FIXNUM)
  (CL:WHEN (CL:= END NULL-INTEGER) (CL:SETQ END (%FILL-POINTER SELF)))
  (CL:RETURN-FROM SUBSEQUENCE (SUBSEQUENCE (%BUFFER SELF) START END)))
