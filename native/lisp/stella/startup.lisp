...
(CL:DECLAIM
  ...
  *CLASS-HIERARCHY-BOOTED?* NIL NULL-INTEGER))

;;; (DEFGLOBAL *STELLA-VERSION* ...)

(CL:DEFVAR *STELLA-VERSION* "3.7.0")
(CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *STELLA-VERSION*))

;;; (DEFGLOBAL *STELLA-MAJOR-VERSION-NUMBER* ...)

(CL:DEFVAR *STELLA-MAJOR-VERSION-NUMBER* NULL-INTEGER)
(CL:DECLAIM (CL:TYPE CL:FIXNUM *STELLA-MAJOR-VERSION-NUMBER*))

;;; (DEFGLOBAL *STELLA-MINOR-VERSION-NUMBER* ...)

(CL:DEFVAR *STELLA-MINOR-VERSION-NUMBER* NULL-INTEGER)
(CL:DECLAIM (CL:TYPE CL:FIXNUM *STELLA-MINOR-VERSION-NUMBER*))

;;; (DEFGLOBAL *STELLA-PATCH-LEVEL* ...)

(CL:DEFVAR *STELLA-PATCH-LEVEL* NULL-INTEGER)
(CL:DECLAIM (CL:TYPE CL:FIXNUM *STELLA-PATCH-LEVEL*))

;;; (DEFGLOBAL *STELLA-RELEASE-STATE* ...)

(CL:DEFVAR *STELLA-RELEASE-STATE* STELLA::NULL-STRING)
...
(CL:DEFUN STELLA-VERSION-STRING ()
  ...
  (CL:RETURN-FROM STELLA-VERSION-STRING
   (CONCATENATE "STELLA " *STELLA-VERSION*)))
...
;;; (DEFGLOBAL *STELLA-GC-VERSION* ...)

(CL:DEFVAR *STELLA-GC-VERSION* "not-installed"
  "Run-time version of the GC library used by STELLA.")
(CL:DECLAIM (CL:TYPE CL:SIMPLE-STRING *STELLA-GC-VERSION*))
...
(CL:DEFUN STARTUP-STARTUP ()
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    (CL:SETQ *STELLA-MAJOR-VERSION-NUMBER*
     (CL:TRUNCATE
      (STRING-TO-INTEGER
       (%WRAPPER-VALUE (NTH (SPLIT-STRING *STELLA-VERSION* #\.) 0)))))
    (CL:SETQ *STELLA-MINOR-VERSION-NUMBER*
     (CL:TRUNCATE
      (STRING-TO-INTEGER
       (%WRAPPER-VALUE (NTH (SPLIT-STRING *STELLA-VERSION* #\.) 1)))))
    (CL:SETQ *STELLA-PATCH-LEVEL*
     (CL:TRUNCATE
      (STRING-TO-INTEGER
       (%WRAPPER-VALUE (NTH (SPLIT-STRING *STELLA-VERSION* #\.) 2)))))
    (CL:LET* ((TEMP-085 (NTH (SPLIT-STRING *STELLA-VERSION* #\.) 3)))
     (CL:SETQ *STELLA-RELEASE-STATE*
      (CL:IF (CL:NOT (CL:EQ TEMP-085 NULL)) (%WRAPPER-VALUE TEMP-085)
       "")))
...
Useful when reporting problems.\" :PUBLIC? TRUE :COMMAND? TRUE)"
    ...
    (DEFINE-FUNCTION-OBJECT "STARTUP"
     "(DEFUN STARTUP ((VERBOSE? BOOLEAN)) :PUBLIC? TRUE :C-CALLABLE? TRUE)"
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 9)
    ...
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     ...
     NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *STELLA-MAJOR-VERSION-NUMBER* INTEGER (STRING-TO-INTEGER (NTH (SPLIT-STRING *STELLA-VERSION* #\\.) 0)) :PUBLIC? TRUE)"
     NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *STELLA-MINOR-VERSION-NUMBER* INTEGER (STRING-TO-INTEGER (NTH (SPLIT-STRING *STELLA-VERSION* #\\.) 1)) :PUBLIC? TRUE)"
     NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *STELLA-PATCH-LEVEL* INTEGER (STRING-TO-INTEGER (NTH (SPLIT-STRING *STELLA-VERSION* #\\.) 2)) :PUBLIC? TRUE)"
     NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *STELLA-RELEASE-STATE* STRING (FIRST-DEFINED (NTH (SPLIT-STRING *STELLA-VERSION* #\\.) 3) \"\") :PUBLIC? TRUE)"
     NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *STELLA-VERSION-STRING* STRING (STELLA-VERSION-STRING) :PUBLIC? TRUE)"
     NULL)
    (DEFINE-GLOBAL-VARIABLE-OBJECT
     "(DEFGLOBAL *STELLA-GC-VERSION* STRING \"not-installed\" :DOCUMENTATION \"Run-time version of the GC library used by STELLA.\" :PUBLIC? TRUE)"
     NULL)
    (CL:LET* ((VERSION 0)) (CL:DECLARE (CL:TYPE CL:FIXNUM VERSION))
     (CL:SETQ *STELLA-GC-VERSION*
      (CONCATENATE
       (INTEGER-TO-STRING
        (CL:TRUNCATE
         (CL:THE CL:FIXNUM
       ...
       "."
       (INTEGER-TO-STRING
        (CL:TRUNCATE (CL:THE CL:FIXNUM (CL:LOGAND VERSION 255)))))))
    (DEFINE-GLOBAL-VARIABLE-OBJECT
