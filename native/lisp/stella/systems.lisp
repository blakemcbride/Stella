...
(CL:DEFUN FIND-FILE-IN-LOAD-PATH-EVALUATOR-WRAPPER (ARGUMENTS)
  (CL:LET*
   ((RESULT
     (%FIND-FILE-IN-LOAD-PATH (%WRAPPER-VALUE (%%VALUE ARGUMENTS))
  ...
  (CL:LET* ((TEST-VALUE-000 (SAFE-PRIMARY-TYPE VALUE)))
   (CL:COND
    ...
    ((SUBTYPE-OF-STRING? TEST-VALUE-000)
     (CL:PROGN
      (CL:COND
       ((STRING-EQUAL? (%WRAPPER-VALUE VALUE) "TRUE")
        (CL:RETURN-FROM COERCE-VALUE-TO-BOOLEAN TRUE-WRAPPER))
       ((STRING-EQUAL? (%WRAPPER-VALUE VALUE) "FALSE")
        (CL:RETURN-FROM COERCE-VALUE-TO-BOOLEAN FALSE-WRAPPER)))))
...
(CL:DEFUN BUILD-STELLA-SYSTEMS-PATH ()
  (CL:LET*
   ...
   (CL:LET*
    ...
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
     (CL:SETQ DIR (%%VALUE ITER-000))
     (CL:WHEN (CL:NOT (BLANK-STRING? (%WRAPPER-VALUE DIR)))
   ...
   (CL:LET* ((DIR NULL) (ITER-001 (SYSTEM-DEFINITION-DIRECTORIES)))
    (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
     ...
     (CL:SETQ ITER-001 (%%REST ITER-001))))
   (CL:LET*
    ((SYS NULL) (ITER-002 (%THE-CONS-LIST *SYSTEMDEFINITIONS*)))
   ...
   (CL:WHEN (EMPTY? EXPLICITPATH)
    (CL:LET*
     ((DIR NULL) (ITER-003 (%THE-CONS-LIST (COPY IMPLICITPATH))))
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-003 NIL)) DO
      (CL:SETQ DIR (%%VALUE ITER-003))
      (CL:WHEN
       (CL:AND (CL:NOT (BLANK-STRING? (%WRAPPER-VALUE DIR)))
        (PROBE-DIRECTORY? (%WRAPPER-VALUE DIR)))
       (CL:LET*
        ((SYSTEMPARENTDIR
          (DIRECTORY-PARENT-DIRECTORY (%WRAPPER-VALUE DIR) 1))
           ...
           (WRAP-STRING
            (FILE-NAME-AS-DIRECTORY
             (CONCATENATE-FILE-NAMES SYSTEMPARENTDIR
              (%WRAPPER-VALUE DIR)))))
          (CL:WHEN
           (CL:AND (PROBE-FILE? (%WRAPPER-VALUE DIR))
            (TOP-LEVEL-STELLA-SYSTEM-DIRECTORY? (%WRAPPER-VALUE DIR)))
     ...
     (CL:WHEN
      ...
      (CL:LET* ((SUBDIR NULL) (ITER-001 RELATIVESYSTEMDIRS))
       (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-001 NIL)) DO
        ...
        (CL:SETQ SYSTEMFILE
         (CONCATENATE-FILE-NAMES (%WRAPPER-VALUE DIR)
          (%WRAPPER-VALUE SUBDIR) SYSTEMFILENAME))
...
of the STELLA installation containing this system based on its definition file."
  (CL:LET* ((SYSDEFFILE (%DEFINITION-FILE SYSTEM)))
    ...
    (CL:LET* ((VALUE-000 NULL))
     ...
     (CL:LET* ((TEMP-000 VALUE-000))
      (CL:LET*
       ...
       (CL:LET* ((TEMP-001 (%SOURCE-ROOT-DIRECTORY REFERENCESYSTEM)))
        (CL:LET*
         ((ROOTSOURCEDEFAULT
         ...
         (CL:LET* ((TEMP-002 (%NATIVE-ROOT-DIRECTORY REFERENCESYSTEM)))
          (CL:LET*
           ((ROOTNATIVEDEFAULT
           ...
           (CL:LET*
            ((TEMP-003 (%BINARY-ROOT-DIRECTORY REFERENCESYSTEM)))
            (CL:LET*
             ((ROOTBINARYDEFAULT
               (TRANSLATE-LOGICAL-PATHNAME
             ...
             (CL:LET*
              ...
              (CL:DECLARE
                ...
                *DEFAULTROOTBINARYDIRECTORY*))
              (CL:WHEN
               (CL:EQ (%SOURCE-ROOT-DIRECTORY SYSTEM)
                STELLA::NULL-STRING)
               (CL:SETF (%SOURCE-ROOT-DIRECTORY SYSTEM)
                (ROOT-SOURCE-DIRECTORY)))
              (CL:WHEN
               (CL:EQ (%NATIVE-ROOT-DIRECTORY SYSTEM)
                STELLA::NULL-STRING)
               (CL:SETF (%NATIVE-ROOT-DIRECTORY SYSTEM)
                (ROOT-NATIVE-DIRECTORY)))
              (CL:WHEN
               (CL:EQ (%BINARY-ROOT-DIRECTORY SYSTEM)
                STELLA::NULL-STRING)
               (CL:SETF (%BINARY-ROOT-DIRECTORY SYSTEM)
                (ROOT-BINARY-DIRECTORY))))))))))))))
   (CL:WHEN
    (CL:OR (CL:EQ (%SOURCE-ROOT-DIRECTORY SYSTEM) STELLA::NULL-STRING)
     (CL:EQ (%NATIVE-ROOT-DIRECTORY SYSTEM) STELLA::NULL-STRING)
     (CL:EQ (%BINARY-ROOT-DIRECTORY SYSTEM) STELLA::NULL-STRING))
    (CL:LET* ((STREAM-000 (NEW-OUTPUT-STRING-STREAM)))
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM-000)
      "Failed to define or compute one or more of `" (%NAME SYSTEM)
      "''s root directories")
     (CL:ERROR (NEW-STELLA-EXCEPTION (THE-STRING-READER STREAM-000))))))
  :VOID)
...
(CL:DEFUN DEFINE-SYSTEM (NAME OPTIONS)
  (CL:LET* ((STRINGNAME STELLA::NULL-STRING))
   ...
   (CL:LET* ((SELF-000 (NEW-SYSTEM-DEFINITION)))
    ...
    (CL:SETF (%DEFINITION-FILE SELF-000) *CURRENTLOADFILE*)
            ...
            (CL:COND
             ...
             ((CL:EQ TEST-VALUE-001 KWD-SYSTEMS-REQUIRED-SYSTEMS)
              ...
              (CL:LET* ((SYS NULL) (ITER-002 VALUE))
               (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-002 NIL)) DO
                (CL:SETQ SYS (%%VALUE ITER-002))
                (GET-SYSTEM-DEFINITION (%WRAPPER-VALUE SYS) CL:T)
                (CL:WHEN
                 (CL:NOT
  ...
  (CL:LET* ((FILES (NEW-LIST)) (FILENAME STELLA::NULL-STRING))
   (CL:LET*
    ((*CURRENTSYSTEMDEFINITION*
      (GET-SYSTEM-DEFINITION (%NAME SYSTEM) CL:T))
     ...
     (CL:LET*
      ...
      (CL:PROGN (INSERT-NEW PROCESSED (WRAP-STRING SYSTEMNAME))
       (CL:WHEN
        (CL:AND RECURSIVE?
         (CL:NOT
          (CL:EQ (%REQUIRED-SYSTEMS *CURRENTSYSTEMDEFINITION*) NULL)))
        (CL:LET*
         ((REQSYS NULL)
          (ITER-000 (%REQUIRED-SYSTEMS *CURRENTSYSTEMDEFINITION*)))
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
          (CL:SETQ REQSYS (%%VALUE ITER-000))
          (CL:WHEN (CL:NOT (MEMBER? PROCESSED REQSYS))
           (%%PRINT-STREAM (%NATIVE-STREAM STANDARD-OUTPUT)
            "Making required system `" (UNWRAP-STRING REQSYS) "'" EOL)
           (%MAKE-SYSTEM (%WRAPPER-VALUE REQSYS)
...
(CL:DEFUN LOAD-PREPROCESSED-FILES (SYSTEMNAME)
  ...
  (CL:WHEN
   (CL:OR (CL:NOT (RUNNING-AS-LISP?))
    (CL:EQ
     (%PREPROCESSED-FILES (GET-SYSTEM-DEFINITION SYSTEMNAME CL:T))
     NULL))
        ...
        (CL:LET*
         ...
         (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
          ...
          (CL:WHEN (CL:NOT (MEMBER? PROCESSED REQSYS))
           ...
           (%LOAD-SYSTEM (%WRAPPER-VALUE REQSYS)
            (CONS-LIST KWD-SYSTEMS-OPTIONS THEOPTIONS)))
          (CL:SETQ ITER-000 (%%REST ITER-000)))))
         ...
         (LIST* SYM-SYSTEMS-STELLA-UNLESS
          ...
          (CONS
           (SYSTEM-STARTUP-FUNCTION-SYMBOL
            (GET-SYSTEM-DEFINITION (%WRAPPER-VALUE SYSTEMNAME) CL:T))
           NIL)
          NIL)
         NIL))
...
(CL:DEFUN COLLECT-STARTUP-FORMS-FROM-SYSTEM-FILE (SYSTEM)
  (CL:LET*
   ((SYSTEMFILENAME (%DEFINITION-FILE SYSTEM)) (MODULE NULL)
    ...
    (CL:PROGN
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
       ...
       (CL:SETQ FILE
        (WRAP-STRING
         (MAKE-FILE-NAME (%WRAPPER-VALUE FILE) KWD-SYSTEMS-STELLA
          CL:T)))
       (CL:WHEN (PROBE-FILE? (%WRAPPER-VALUE FILE))
        (CL:WHEN
         (CL:OR (CL:EQ DATE NULL)
          (GREATER? (FILE-WRITE-DATE (%WRAPPER-VALUE FILE)) DATE))
         (CL:SETQ DATE (FILE-WRITE-DATE (%WRAPPER-VALUE FILE)))))
       (CL:SETQ ITER-000 (%%REST ITER-000))))
...
variable or on its most recent modification date of any of its source files."
  (CL:LET*
   ((NAME (%NAME SYSTEM))
    (MODULE (GET-STELLA-MODULE (%CARDINAL-MODULE SYSTEM) CL:NIL))
    (VERSIONSYMBOLNAME STELLA::NULL-STRING) (VERSIONSYMBOL NULL)
    (VERSIONVARIABLE NULL)
    (VERSIONSTRING
     (%WRAPPER-VALUE
      (DYNAMIC-SLOT-VALUE (%DYNAMIC-SLOTS SYSTEM)
    ...
    (CL:LET*
     ...
     (CL:LOOP WHILE (CL:NOT (CL:EQ ITER-000 NIL)) DO
      ...
      (CL:SETQ VERSIONSYMBOLNAME
       (REPLACE-SUBSTRINGS (%WRAPPER-VALUE PATTERN) NAME "<name>"))
      ...
      (CL:WHEN (CL:NOT (CL:EQ VERSIONSYMBOL NULL))
       ...
       (CL:WHEN
        ...
        (CL:SETQ VERSIONSTRING
         (%WRAPPER-VALUE (READ-GLOBAL-VARIABLE-VALUE VERSIONVARIABLE)))
   ...
   (CL:RETURN-FROM GET-SYSTEM-VERSION-STRING
    (%WRAPPER-VALUE (LAST (SPLIT-STRING VERSIONSTRING #\ ))))))
