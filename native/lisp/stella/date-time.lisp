...
(CL:DEFUN FORMAT-DATE-PRINT-TWO-DIGIT-NUMBER (NUMBER FLAG STREAM)
  ...
  (CL:WHEN (CL:<= NUMBER 9) (CL:WHEN (NULL? FLAG) (CL:SETQ FLAG #\0))
   (CL:CASE FLAG (#\_ (%%PRINT-STREAM (%NATIVE-STREAM STREAM) " "))
    ((#\0 NULL-CHARACTER) (%%PRINT-STREAM (%NATIVE-STREAM STREAM) "0"))
    (CL:OTHERWISE)))
...
(CL:DEFUN FORMAT-DATE-ABBREVIATED-WEEKDAY-HANDLER (DATE CONT FLAG STREAM)
  (CL:SETQ CONT CONT)
  (FORMAT-DATE-PRINT-STRING (%SYMBOL-NAME (%DATE-TIME-DOW DATE)) FLAG 0
   3 STREAM)
  :VOID)
  ...
  (FORMAT-DATE-PRINT-STRING
   (%WRAPPER-VALUE
    (CL:AREF
     ...
     (CL:THE CL:FIXNUM (%DATE-TIME-MONTH DATE))))
   FLAG 0 -1 STREAM)
  :VOID)
...
(CL:DEFUN FORMAT-DATE-24-HOUR-HANDLER (DATE CONT FLAG STREAM)
  (CL:SETQ CONT CONT)
  (FORMAT-DATE-PRINT-TWO-DIGIT-NUMBER (%DATE-TIME-HOUR DATE) FLAG
   STREAM)
  :VOID)
...
(CL:DEFUN FORMAT-DATE-12-HOUR-HANDLER (DATE CONT FLAG STREAM)
  (CL:SETQ CONT CONT)
  (CL:LET* ((HOUR (CL:1+ (MOD (CL:1- (%DATE-TIME-HOUR DATE)) 12))))
...
(CL:DEFUN FORMAT-DATE-MONTH-HANDLER (DATE CONT FLAG STREAM)
  (CL:SETQ CONT CONT)
  (FORMAT-DATE-PRINT-TWO-DIGIT-NUMBER (%DATE-TIME-MONTH DATE) FLAG
   STREAM)
  :VOID)
...
(CL:DEFUN FORMAT-DATE-MINUTE-HANDLER (DATE CONT FLAG STREAM)
  (CL:SETQ CONT CONT)
  (FORMAT-DATE-PRINT-TWO-DIGIT-NUMBER (%DATE-TIME-MINUTE DATE) FLAG
   STREAM)
  :VOID)
...
(CL:DEFUN FORMAT-DATE-AM-PM-HANDLER (DATE CONT FLAG STREAM)
  (CL:SETQ CONT CONT)
  (CL:LET* ((AMPM (CL:IF (CL:< (%DATE-TIME-HOUR DATE) 12) "AM" "PM")))
...
(CL:DEFUN FORMAT-DATE-SECOND-HANDLER (DATE CONT FLAG STREAM)
  (CL:SETQ CONT CONT)
  (FORMAT-DATE-PRINT-TWO-DIGIT-NUMBER (%DATE-TIME-SECOND DATE) FLAG
   STREAM)
  :VOID)
...
(CL:DEFUN FORMAT-DATE-TWO-DIGIT-YEAR-HANDLER (DATE CONT FLAG STREAM)
  (CL:PROGN (CL:SETQ CONT CONT) (CL:SETQ FLAG FLAG))
  (CL:LET* ((YEAR (MOD (%DATE-TIME-YEAR DATE) 100)))
  ...
  (CL:WHEN (CL:< (%DATE-TIME-YEAR DATE) 1000)
   ...
   (CL:CASE FLAG
    (#\_
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM)
      (FORMAT-WITH-PADDING
       (INTEGER-TO-STRING (CL:TRUNCATE (%DATE-TIME-YEAR DATE))) 4 #\
       KWD-DATE-TIME-RIGHT CL:NIL))
     (CL:RETURN-FROM FORMAT-DATE-YEAR-HANDLER))
    ((#\0 NULL-CHARACTER)
     (%%PRINT-STREAM (%NATIVE-STREAM STREAM)
      (FORMAT-WITH-PADDING
       (INTEGER-TO-STRING (CL:TRUNCATE (%DATE-TIME-YEAR DATE))) 4 #\0
       KWD-DATE-TIME-RIGHT CL:NIL))
     (CL:RETURN-FROM FORMAT-DATE-YEAR-HANDLER))
    (CL:OTHERWISE)))
...
(CL:DEFUN FORMAT-DATE-NUMERIC-TIME-ZONE-HANDLER (DATE CONT FLAG STREAM)
  (CL:SETQ CONT CONT)
  (%%PRINT-STREAM (%NATIVE-STREAM STREAM)
   (TIME-ZONE-FORMAT60 (%DATE-TIME-ZONE DATE) (CL:EQL FLAG #\:)))
  :VOID)
   ...
   (CL:WHEN (CL:NOT (CL:EQ TIMEZONE NULL))
    (CL:LET* ((DECZONE (ZONE TIMEZONE)))
     (CL:WHEN (CL:AND (CL:EQ DECZONE NULL) (STRING? TIMEZONE))
      (CL:SETQ DECZONE
       (ZONE
        (WRAP-STRING (STRING-DOWNCASE (%WRAPPER-VALUE TIMEZONE))))))
     (CL:WHEN (CL:NOT (CL:EQ DECZONE NULL))
      (CL:SETQ DECODEDZONE (%WRAPPER-VALUE DECZONE)))))
     ...
     (CL:COND
      ((CL:EQL CH #\%)
       ...
       (CL:FUNCALL
        (%WRAPPER-VALUE
         (CL:AREF
          (CL:THE CL:SIMPLE-VECTOR (%THE-ARRAY *FORMAT-DATE-HANDLERS*))
          (CL:THE CL:FIXNUM (CL:CHAR-CODE CH))))
  ...
  (CL:LET* ((*MODULE* *STELLA-MODULE*) (*CONTEXT* *MODULE*))
   ...
   (CL:WHEN (CURRENT-STARTUP-TIME-PHASE? 4)
    ...
    (CL:LET* ((SELF-040 (NEW-DECODED-DATE-TIME)))
     (CL:SETF (%DATE-TIME-MILLIS SELF-040) NULL-FLOAT)
     (CL:SETF (%DATE-TIME-SECOND SELF-040) NULL-INTEGER)
     (CL:SETF (%DATE-TIME-MINUTE SELF-040) NULL-INTEGER)
     (CL:SETF (%DATE-TIME-HOUR SELF-040) NULL-INTEGER)
     (CL:SETF (%DATE-TIME-DAY SELF-040) NULL-INTEGER)
     (CL:SETF (%DATE-TIME-MONTH SELF-040) NULL-INTEGER)
     (CL:SETF (%DATE-TIME-YEAR SELF-040) NULL-INTEGER)
     (CL:SETF (%DATE-TIME-ZONE SELF-040) NULL-FLOAT)
     (CL:SETF (%DATE-TIME-DOW SELF-040) NULL)
    ...
    (CL:LET* ((TABLE *FORMAT-DATE-HANDLERS*))
     (CL:LET*
      ...
      (CL:LOOP WHILE (CL:<= ITER-042 UPPER-BOUND-043) DO
       (CL:SETQ I ITER-042)
       (CL:LET
        ((SELF (%THE-ARRAY *FORMAT-DATE-HANDLERS*))
         (VALUE
